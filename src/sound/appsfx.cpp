...
typedef struct {
   ...
   const char  *name;      // for streaming sounds, i guess, for now, maybe
...
void _sfx_basic_setup(int which, _sfx *newfx, int type, sfx_parm *parm, const char *sfx_name)
{
   if (parm)
      newfx->parm=*parm;
   else
...
int SFX_Play_Raw(int type, sfx_parm *parm, const char *sfx_name)
{
   if (in_kill)
      return SFX_NO_HND;
...
int SFX_Play_Obj(int type, sfx_parm *parm, const char *sfx_name, int objid)
{
   int hnd=SFX_Get();
#ifndef SHIP
   if (!ObjExists(objid))
...
int SFX_Play_Vec(int type, sfx_parm *parm, const char *sfx_name, mxs_vector *vector)
{
   int hnd=SFX_Get();
   if (hnd!=SFX_NO_HND)
   {
